...
export default function UserRoleManagement({
  ...
  } = useGetMembersSwitcher(
    { userId: safeUserId },
  );

  // Derived state
  const allowedWorkspaceIds = useMemo(
    () => memberRecords.map((m: any) => m.workspaceId),
    [memberRecords]
  );

  const allowedWorkspaces = useMemo(
    () =>
      allWorkspaces.filter((ws: Workspace) =>
        allowedWorkspaceIds.includes(ws.id)
      ),
    [allWorkspaces, allowedWorkspaceIds]
  );
  ...
  const handleUpdateRole = () => {
    ...
    const confirmationMessage = `Are you sure you want to change this user's role to ${formatRole(
      selectedRole
    )}?`;
  ...
  return (
    <div className="space-y-6">
      {/* Loading state */}
      {isWorkspacesPending || isMembersPending ? (
        ...
        <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="p-4 bg-sidebar-accent/10 rounded border border-sidebar-accent/20">
                  ...
                  <div className="ml-7">
                    <Badge
                      variant="secondary"
                      className="px-3 py-1 h-auto text-base"
                    >
          ...
          <Card className="bg-sidebar border-[var(--sidebar-border)]">
            ...
            <CardContent className="space-y-6">
              ...
              {/* Workspace Selector */}
              {selectedRole &&
                needsWorkspaceChange(user.role, selectedRole) && (
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-sidebar-foreground">
                      Select Workspace
                    </label>
                    ...
                    >
                      ...
                      <SelectContent className="bg-sidebar border-sidebar-accent">
                        {allowedWorkspaces.map((workspace: any) => (
                          <SelectItem
                            key={workspace.id}
                            value={workspace.id}
                            className="hover:bg-sidebar-accent/20"
                    ...
                    <p className="text-xs text-sidebar-foreground/60 mt-1">
                      Select the workspace where this user will have the new
                      role
                    </p>
                  </div>
                )}
              ...
              <div className="mt-6 p-4 bg-sidebar-accent/10 rounded border border-sidebar-accent/20">
                ...
                <ul className="space-y-2 text-sm text-sidebar-foreground/70">
                  <li>
                    • Changing a user's role may affect their access to features
                    and data
                  </li>
                  <li>
                    • Some role changes require moving the user to a different
                    workspace
                  </li>
                  <li>
                    • The user will need to log out and back in for changes to
                    take effect
                  </li>
                  <li>
                    • Super Admin role should be granted sparingly to maintain
                    security
                  </li>
                </ul>
              </div>
